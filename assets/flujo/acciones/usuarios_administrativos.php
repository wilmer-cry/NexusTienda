...
if ($registrosPorPagina === 0) {
    ...
    $sql = "SELECT id, nombre, usuario, id_cargo, codigo, correo, status FROM usuarios WHERE status='1' AND id_cargo='1'";
    $result = $mysqli->query($sql);
} else {
    
    $sql = "SELECT id, nombre, usuario, id_cargo, codigo, correo, status FROM usuarios  WHERE status='1' AND id_cargo='1' LIMIT $inicio, $registrosPorPagina";
    $result = $mysqli->query($sql);
}
